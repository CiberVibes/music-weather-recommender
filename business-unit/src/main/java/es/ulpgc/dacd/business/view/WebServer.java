...
public class WebServer {
    ...
    public void start() {
        ...
        app.get("/api/locations", this::handleLocations);
        app.get("/api/recommendations", this::handleRecommendations);
        app.get("/auth/login", this::handleLogin);
        app.get("/callback", this::handleCallback);
        app.get("/api/token", this::handleToken);
        app.get("/api/search", this::handleSearch);
        app.post("/api/play", this::handlePlay);

        app.start(PORT);
        logger.info("Web UI available at http://localhost:" + PORT);
    }

    private void handleLocations(Context ctx) {
    ...
    private void handleRecommendations(Context ctx) {
    ...
    private void handleCallback(Context ctx) {
        ...
        if (code != null && spotify != null) {
            try {
                ...
                logger.warning("OAuth error: " + e.getMessage());
            }
        }
        ctx.redirect("/");
    }
    ...
    private void handleToken(Context ctx) {
        if (spotify == null || !spotify.isAuthorized()) { ctx.status(401).result("Not authorized"); return; }
        try {
    ...
    private void handlePlay(Context ctx) {
        ...
        try {
            ...
            String deviceId = body.get("deviceId").getAsString();
            String uri = body.get("uri").getAsString();
            int status = spotify.playTrack(deviceId, uri);
